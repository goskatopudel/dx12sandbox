...
GPUFenceHandle ParallelRenderScene(GPUQueue* queue, Scene &Scene, GPUCommandList* drawCmds, ICameraControler* pCamera) {
	Check(IsMainThread());
	...
	auto fence = GetFence(Back(childWorkspaces).CommandList);
