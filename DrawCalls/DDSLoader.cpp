...
//
//
//static LoadResult CreateTextureFromDDS(
...
//
//	Array<D3D12_SUBRESOURCE_DATA> initData(Memory::defaultAllocator());
...
//	size_t tdepth = 0;
//
//	FillInitData(width, height, depth, mipCount, arraySize, format, maxsize, bitSize, bitData, twidth, theight, tdepth, skipMip, initData.data_);
//
...
//	
//
//	return{ { createResourceAndUploadData(resourceDesc, initData.data_, (uint32_t)size(initData)),
//		resourceDesc, srvDimension }, R_OK };
//}
//
//
//LoadResult LoadDDSFromMemory(const byte * ddsData, size_t ddsDataSize) {
//	if (!ddsData)
//	{
//		return{ {}, R_CORRUPT_DATA };
//	}
...
//
//	return CreateTextureFromDDS(header, ddsData + offset, ddsDataSize - offset, 0);
//}
//
////LoadResult LoadDDSFromFile(TextId file) {
////	auto loaded = FileIO::read(Strings::cStr(file));
////
////	if (loaded.isOk()) {
////		auto result = loadDDSFromMem((const byte*)loaded.blob.ptr, loaded.blob.size);
////		FileIO::freeBlob(loaded.blob);
////		return result;
////	}
////	else {
